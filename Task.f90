module Task
  ...
  subroutine GetMaxCoordinates(A, x1, y1, x2, y2)
    ...
    ! Значение private переменных требует инициализации, поэтому начало до
    ! allocate(current_column(m)). Знач.мас. А сохраняется при выходе из пар.региона.
    !____________________________________________________________________________________
    ...
    ! Размер части по умолчанию (chunk-size): 1. Стоит ли использовать auto?
    !____________________________________________________________________________________
    ...
    do L = 1, n
      ...
      do R = L, n
        ...
        ! СИНХРОНИЗАЦИЯ ПОТОКОВ С КОНСТРУКЦИЕЙ CRITICAL
        ! Эта часть должна выполняться ТОЛЬКО одним потоком! Для этого можно и нужно
        ! использовать средства синхронизации. Создадим критическую секцию (без имени,
        ! т.к. она одна и поэтому независимая). В эту область может зайти только один
        ! поток, остальные приостанавливаются.
        !$omp critical
